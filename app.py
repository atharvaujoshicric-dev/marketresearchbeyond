...
def send_email(recipient_email, excel_data, filename):
    try:
        ...
        msg['Subject'] = "Your Property Analysis Report"
...
if uploaded_file:
    ...
    if desc_col and cons_col and prop_col:
        with st.spinner('Calculating...'):
            ...
            # 2. Ensure calculation columns are at the very end of Raw Data
            ...
            if st.button("Send to Email"):
                if recipient:
                    with st.spinner("Sending email..."):
                        if send_email(recipient, processed_data, "Property_Analysis_Report.xlsx"):
                            st.success(f"Report sent successfully to {recipient}!")
                            st.balloons()
                else:
                    st.error("Please enter a recipient email.")
    else:
        st.error("Column Check Failed: Ensure your Excel has 'Property', 'Property Description', and 'Consideration Value'.")
