...
def extract_area_logic(text):
    ...
    for match in re.finditer(rf'(\d+\.?\d*)\s*{m_unit}', text, re.IGNORECASE):
        ...
        if 1.0 <= val < 600 and (any(word in context_before for word in include_keywords) or not any(word in context_before for word in exclude_keywords)):
            m_vals.append(val)
    ...
    for match in re.finditer(rf'(\d+\.?\d*)\s*{f_unit}', text, re.IGNORECASE):
        val = float(match.group(1))
        context_before = text[max(0, match.start()-60):match.start()].lower()
        if 10.0 <= val < 6000 and (any(word in context_before for word in include_keywords) or not any(word in context_before for word in exclude_keywords)):
            f_vals.append(val)
    if f_vals:
        if ("अपार्टमेंट" in text or "सदनिका" in text) and "येथील" not in text: return round(f_vals[0] / 10.764, 3)
    ...
    if is_summary:
        for i in range(2, len(df) + 2):
            ...
            if curr_prop != next_prop:
                ...
                color_idx += 1
                start_row_prop = i + 1
            curr_cfg_key = [df.iloc[i-2, 0], df.iloc[i-2, 2]] # Prop + Config
...
if uploaded_file:
    ...
    if desc_col and cons_col and prop_col and date_col:
        with st.spinner('Calculating...'):
            ...
            # Ensure Date column is in datetime format for accurate 'Max' selection
            ...
            # Grouping Summary to show the LAST completion date per Property & Config
            summary = valid_df.groupby([prop_col, 'Configuration', 'Carpet Area (SQ.FT)']).agg(
                Last_Completion_Date=(date_col, 'max'), # Picks the latest date
                Min_APR=('APR', 'min'), Max_APR=('APR', 'max'), Avg_APR=('APR', 'mean'),
                Property_Count=(prop_col, 'count')
            ).reset_index()
            
            summary = summary[['Property', 'Last_Completion_Date', 'Configuration', 'Carpet Area(SQ.FT)', 'Min_APR', 'Max_APR', 'Avg_APR', 'Property_Count']]
            summary['Last_Completion_Date'] = summary['Last_Completion_Date'].dt.strftime('%d-%m-%Y') # Clean date format
            summary.columns = ['Property', 'Last Completion Date', 'Configuration', 'Carpet Area(SQ.FT)', 'Min. APR', 'Max APR', 'Average of APR', 'Count of Property']
            ...
            if st.button("Send to Email") and recipient:
                if send_email(f"{recipient.strip().lower()}@beyondwalls.com", output.getvalue(), "Spydarr_Report.xlsx"):
                    st.success("Sent!")
    else:
