...
# --- EMAIL CONFIGURATION ---
# NOTE: hard-coding an app password in source is a security risk if this file
# is ever shared/committed. Prefer st.secrets["APP_PASSWORD"] in production.
...
def extract_area_logic(text, is_carpet_type=False):
    """
    Extracts the carpet area (in SQ.MT) from a Marathi/Hindi property
    description.
    ...
    only the base carpet figure is used (component values under 2.0 that look
    like a real balcony are still folded in, matching legacy behaviour).
    """
    ...
    for match in re.finditer(rf'(\d+\.?\d*)\s?{m_unit}', relevant_text, re.IGNORECASE):
        ...
        if not any(word in context_before for word in exclude_keywords):
            if is_carpet_type:
                ...
                # Usable Carpet / blank rows: only fold in "small" component-looking values.
    ...
    # 5. IMPERIAL SUMMATION FALLBACK (converted to SQ.MT for the final value)
    f_vals = []
...
def normalize_area_type(val):
    """
    Normalize the 'Area Type' column into one of:
      - 'carpet'         -> plain Carpet
      - 'usable_carpet'  -> Usable/Useable Carpet
      - ''                -> blank / unrecognized
    """
    if pd.isna(val):
        return ''
...
def resolve_area_types(df, area_type_col, prop_col):
    ...
    norm_area_type = df[area_type_col].apply(normalize_area_type)

    helper = pd.DataFrame({'prop': df[prop_col], 'norm': norm_area_type})
    ...
    resolved = norm_area_type.copy()
    blank_mask = resolved == ''
    resolved.loc[blank_mask] = df.loc[blank_mask, prop_col].map(property_mode_map).fillna('')
    return resolved
...
def load_input_dataframe(uploaded_file):
    """
    Reads the uploaded file. For .xlsx files, looks specifically for a sheet
    named 'in' (case-insensitive). Falls back to letting the user pick a
    sheet if 'in' isn't found. CSV files are read as-is (no sheet concept).
    """
    ...
    if 'in' in sheet_map:
        return pd.read_excel(xls, sheet_name=sheet_map['in'])

    st.warning("No sheet named 'in' was found in this workbook. Please choose the sheet to use.")
    chosen = st.selectbox("Sheet to read", xls.sheet_names)
    return pd.read_excel(xls, sheet_name=chosen)
...
if uploaded_file:
    df = load_input_dataframe(uploaded_file)
    ...
    if desc_col and cons_col and prop_col and date_col and loc_col:
        with st.spinner('Calculating...'):
            resolved_area_type = resolve_area_types(df, area_type_col, prop_col)
            df['Carpet Area (SQ.MT)'] = [
                extract_area_logic(desc, rt == 'carpet')
                for desc, rt in zip(df[desc_col], resolved_area_type)
            ]
        ...
        st.error("Missing required columns. Ensure the sheet has 'Micromarket', 'Property Description', 'Consideration Value', 'Property', and 'Completion Date'.")
